...
function updateOneSignalTags(prefs) {
  ...
  OneSignalDeferred.push(async function(OneSignal) {
    ...
    // 1. On demande la permission (Indispensable sur iOS)
    // Cela fera apparaître la petite fenêtre native de Safari
    await OneSignal.Notifications.requestPermission();

    // 2. On vérifie si l'utilisateur est maintenant bien abonné
    console.log("L'utilisateur est abonné :", OneSignal.Notifications.permission);

    // 3. On envoie les tags
    const tags = {};
    ALL_THEMES.forEach(t => {
      tags[t.id] = prefs.includes(t.id) ? "true" : "false";
    });
    
    console.log("Envoi des tags vers OneSignal...", tags);
