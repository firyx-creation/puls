...
function initSetup() {
  ...
  document.getElementById("btn-start").addEventListener("click", async () => {
    ...
    STATE.pseudo = pseudo;
    STATE.prefs  = prefs;
    STATE.isAdmin = pseudo === ADMIN_PSEUDO;
    ...
    if (document.getElementById("toggle-notif").checked) {
      await setupNtfyNotifications(prefs);
    }
...
//  Principe : chaque thème = un topic ntfy unique
//  L'utilisateur s'abonne aux topics de ses thèmes via l'app ntfy
//  L'admin envoie un POST HTTP à ntfy.sh quand il publie
// ══════════════════════════════════════
async function setupNtfyNotifications(prefs) {
  // On vérifie si le navigateur supporte les notifs
  if (!("Notification" in window)) {
    setNotifStatus("Notifications non supportées.", "err");
    return;
  }

  const perm = await Notification.requestPermission();
  if (perm !== "granted") {
    setNotifStatus("Permission refusée.", "err");
    return;
  }

  // Enregistre le service worker ntfy
  try {
    if ("serviceWorker" in navigator) {
      await navigator.serviceWorker.register("/sw.js");
    }
    // Abonne le navigateur aux topics ntfy via Web Push (si supporté)
    // Sinon on affiche les instructions pour l'app ntfy
    const topics = prefs.map(p => NTFY_TOPICS[p]).filter(Boolean);
    showNtfyInstructions(topics, prefs);
    setNotifStatus("✓ Notifications configurées !", "ok");
  } catch(e) {
    console.error("Notif setup error:", e);
    setNotifStatus("Erreur notifications.", "err");
  }
}
...
async function publishPost() {
  ...
  try {
    ...
    // Envoie la notification push via ntfy.sh si demandé + post immédiat
    if (notifyUsers && STATE.schedType === "now") {
      await sendNtfyNotification(postData);
    }
